...
JOIN public.film f ON (
    ...
    (f.title = 'The Dark Knight' AND a.last_name IN ('BALE','LEDGER')) OR
    (f.title = 'Interstellar' AND a.last_name IN ('MCCONAUGHEY','HATHAWAY'))
)
WHERE NOT EXISTS (
...
WHERE customer_id IN (
    SELECT customer_id FROM deleted_rentals
);
...
INSERT INTO public.rental (
    rental_date, inventory_id, customer_id, staff_id, return_date, last_update
)
SELECT 
    current_date,
    i.inventory_id,
    c.customer_id,
    s.staff_id,
    current_date + f.rental_duration,
    current_date
...
JOIN public.staff s ON s.store_id = st.store_id
WHERE f.title IN ('Inception','The Dark Knight','Interstellar')
AND NOT EXISTS (
...
SELECT 
    ...
    r.staff_id,
    r.rental_id,
    CASE WHEN (r.return_date - r.rental_date) <= f.rental_duration
    	THEN f.rental_rate
    	WHEN (r.return_date - r.rental_date) <= f.rental_duration * 2
    	THEN f.rental_rate + ((r.return_date - r.rental_date - f.rental_duration) * 1)
