...
from auction_locations l, 
join employees e ON e.role = 'Manager'
where l.address = 'Tbilisi'
limit 1;

insert into lots (event_id, lot_number, starting_price)
select ae.event_id, 1, 100
from auction_events ae
join auction_locations l on ae.location_id = l.location_id
where l.address = 'Tbilisi'
...
-- Validation
select 'participants' as table_name, COUNT(*) from participants where record_ts is null
union all
select 'items', COUNT(*) from items where record_ts is null
union all
select 'lots', COUNT(*) from lots where record_ts is null
union all
select 'bids', COUNT(*) from bids where record_ts is null
union all
